...
chmod -R 777 "$FILESHARE_DIR"
chmod -R 777 "$MUSIC_DIR"
...
<body>
    ...
    <div id="content">
        ...
        <ul>
            ...
            <li><a href="http://$SERVER_IP:8000">Knowledge Base Documents</a></li>
            <li><a href="http://$SERVER_IP:8001">Listen To Music</a></li>
        </ul>
    </div>
</body>
</html>
...
# Configure Apache to serve the fileshare on port 8000 and music on port 8001
cat <<EOL > $APACHE_CONF
<VirtualHost *:80>
    ...
    Alias /fileshare /opt/fileshare
    <Directory /opt/fileshare>
        Options Indexes FollowSymLinks
        AllowOverride None
        Require all granted
    </Directory>
    Alias /music /opt/music
    <Directory /opt/music>
        Options Indexes FollowSymLinks
        AllowOverride None
        Require all granted
...
# Create a new Apache site configuration for serving music on port 8001
cat <<EOL > /etc/apache2/sites-available/music.conf
<VirtualHost *:8001>
    ServerAdmin webmaster@localhost
    DocumentRoot /opt/music
    <Directory /opt/music>
        Options Indexes FollowSymLinks
        AllowOverride None
        Require all granted
...
echo "Access the chat at http://$SERVER_IP:3000"
echo "Access the fileshare at smb://$SERVER_IP/fileshare and http://$SERVER_IP:8000"
echo "Access the music at http://$SERVER_IP:8001"
